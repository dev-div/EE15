...
class TestInvoiceExtract(AccountTestInvoicingCommon, account_invoice_extract_common.MockIAP):
    ...
    def test_fiscal_position(self):
        """
        test that the fiscal position of an existing partner is set on invoice
        """
        ...
        existing_partner = self.env['res.partner'].create({'name': 'Test', 'property_account_position_id': fiscal_pos.id})
        ...
        self.assertEqual(invoice.partner_id, existing_partner)
        self.assertEqual(invoice.fiscal_position_id, existing_partner.property_account_position_id)
